# Привет, меня зовут Дмитрий! <img src="https://github.com/TheDudeThatCode/TheDudeThatCode/blob/master/Assets/Hi.gif" width="35" />

### <img src="https://github.com/TheDudeThatCode/TheDudeThatCode/blob/master/Assets/Developer.gif" width="45" /> Обо мне: я **QA-инженер** (ручное и автоматизированное тестирование). Люблю находить баги там, где их быть не должно, и делать продукты качественнее

- 🤖 Пишу автотесты на Python
- 🔍 Занимаюсь функциональным, регрессионным и интеграционным тестированием
- 📈 Постоянно улучшаю процессы и документацию (чек-листы, тест-кейсы, баг-репорты)
---

## 🚀 Мои пет-проекты (AQA)

Здесь собраны проекты, которые я сделал для практики и души:

| Проект                                                      | Описание                                                                                                                                                                    |
|:------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **[Saucedemo](https://github.com/ddfomin/saucedemo)**       | *UI-автотесты интернет-магазина Saucedemo: POM, логирование, Faker, CI-пайплайн, Allure-отчёты.*                                                                            |
| **[GoogleMapAPI](https://github.com/ddfomin/GoogleMapAPI)** | *Автотесты API геоданных (аналог Google Maps API). Покрыты CRUD-операции и полный жизненный цикл локаций. Реализованы CI-пайплайн на GitHub Actions и логирование запросов* |

## 🛠 Стек

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Pytest](https://img.shields.io/badge/Pytest-0A9EDC?style=for-the-badge&logo=pytest&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white)
![Requests](https://img.shields.io/badge/Requests-2CA5E0?style=for-the-badge&logo=python&logoColor=white)
![Allure](https://img.shields.io/badge/Allure-FF6C37?style=for-the-badge&logo=testinglibrary&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
